...
func _ready():
	...
	if socketUDP.bind(listenPort) != OK:
		print("GameServer LAN service: Tidak dapat mengakses port: " + str(listenPort))
	else:
		#print("GameServer LAN service: Mendeteksi koneksi pada port: " + str(listenPort))
		pass
	...
	if socketUDP.get_available_packet_count() > 0:
		...
		if serverIp != '' and serverPort > 0:
			...
			if not knownServers.has(serverIp):
				...
				#print("Menemukan server: %s - %s:%s" % [gameInfo.nama, gameInfo.ip, gameInfo.max_pemain])
...
func clean_up():
	...
	for serverIp in knownServers:
		...
		if (now - serverInfo.lastSeen) > server_cleanup_threshold:
			...
			#print('Menghapus server: %s' % serverIp)
