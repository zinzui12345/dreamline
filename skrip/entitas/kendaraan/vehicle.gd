...
## Apply Stability damping even if all wheel touch ground.
## this must be enabled on thin vehicle like bike
@export var continuous_damping := false
...
func initialize():
	...
	set("center_of_mass_mode", RigidBody3D.CENTER_OF_MASS_MODE_CUSTOM)
	set("mass", vehicle_mass)
	...
	set("center_of_mass", center_of_gravity)
...
func process_drag() -> void:
	...
	if drag > 0.0:
		call("apply_central_force", -local_velocity.normalized() * drag)
...
func process_stability() -> void:
	...
	if enable_stability:
		...
		if plane_xz.y < 0 and plane_xz.length() > 1.0:
			...
			if yaw_angle > stability_yaw_engage_angle and signf(get_indexed("angular_velocity:y")) == signf(plane_xz.x):
				stability_yaw_torque = (yaw_angle - stability_yaw_engage_angle) * stability_yaw_strength
				stability_yaw_torque *= vehicle_inertia.y * clampf(absf(get_indexed("angular_velocity:y")) - 0.5, 0.0, 1.0)
		
		stability_torque_vector = Vector3.ZERO
		if get_wheel_contact_count() < 3 or continuous_damping:
			stability_torque_vector = (global_transform.basis.y.cross(Vector3.UP) * vehicle_inertia * stability_upright_spring) + (-get("angular_velocity") * stability_upright_damping)
			call("apply_torque", stability_torque_vector)
		else:
		...
		if stability_yaw_torque:
			...
			call("apply_torque", global_transform.basis.y * stability_yaw_torque)
