...
@export var left_block : Node :
	set(node):
		if node != null:
			$MarginContainer/arithmetic_operator/left_block.add_child(node)
		else:
			$MarginContainer/arithmetic_operator/left_block.remove_child(left_block)
		left_block = node
@export var right_block : Node :
	set(node):
		if node != null:
			$MarginContainer/arithmetic_operator/right_block.add_child(node)
		else:
			$MarginContainer/arithmetic_operator/right_block.remove_child(right_block)
		right_block = node
