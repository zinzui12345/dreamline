...
const BlockTreeUtil = preload("res://skrip/editor kode/block_tree_util.gd")
...
const Util = preload("res://skrip/editor kode/util.gd")
...
func block_script_selected(block_script: BlockScriptSerialization, node : Node = server.permainan.edit_objek):
	clear_canvas()
...
func load_tree(parent: Node, node: BlockSerialization):
	...
	for c in node.path_child_pairs:
		if scene.get_node_or_null(c[0]) == null:
			push_error("invalid node path : " + str(c[0]))
			continue
		load_tree(scene.get_node(c[0]), c[1])


func rebuild_block_trees(undo_redo = null):
